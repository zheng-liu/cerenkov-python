...
g_cluster_info_list <- if (! g_par$flag_create_ec2_instances) {
    ...
    c(g_create_ec2_instances(g_ec2_par,
                             g_run_ec2_instance,
...
if (g_par$flag_create_fork_cluster) {
    clusterExport(g_cluster, varlist=list("g_make_make_temp_dir_if_doesnt_exist", "g_get_temp_directory"))
    g_make_temp_dir_if_doesnt_exist <- g_make_make_temp_dir_if_doesnt_exist(g_get_temp_directory)
...
g_classifier_runner_func_use <- if (! is.null(g_cluster)) {
...
g_output_file_name <- sprintf("%s_%s_%d.Rdata",
