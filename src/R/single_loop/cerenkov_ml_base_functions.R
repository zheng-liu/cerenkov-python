...
# ------------- debugging code ------------
#                print(sprintf("starting classifiers for replication %d, fold %d at date/time: %s", replication_id, fold_id, Sys.time()))
# ------------- debugging code ------------

                ml_single_results_list <- p_func_lapply_second_level(1:length(p_classifier_list),
                       function(p_classifier_list_index) {
                           ## get the classifier
                           p_classifier <- p_classifier_list[[p_classifier_list_index]]

                           ## get the integer classifier ID
                           classifier_id <- names(p_classifier_list)[p_classifier_list_index]

# ------------- debugging code ------------
###                         print(sprintf("Starting classifier ID %s", p_classifier$classifier_set_name))
# ------------- debugging code ------------

                           ## need to know the feature matrix name, so we can retrieve the feature matrix
                           classifier_feature_matrix_name <- p_classifier$classifier_feature_matrix_name

                           ## need the classifier's hyperparameter list
                           classifier_hyperparameter_list <- p_classifier$classifier_hyperparameter_list

                           ## need the classifier's hyperparameter set name, which dictates what top-level list slot the results go into
                           classifier_hyperparameter_set_type_name <- p_classifier$classifier_hyperparameter_set_type_name

                           ## need the classifier's function name so we can look up the classifier function
                           classifier_function_name <- p_classifier$classifier_function_name
                           ...
                           if (is.null(p_classifier$feature_reducer_function_name)) {
                               ...
                               if (! is.null(classifier_feature_matrix_name)) {
                                ...
                                if (is.null(base_feature_matrix)) { stop(sprintf("feature matrix %s missing",
                                                                                 classifier_feature_matrix_name)) }
                                feature_reducer_function_name <- p_classifier$feature_reducer_function_name
                                ...
                                feature_reducer_input_matrix_name <- p_classifier$feature_reducer_input_matrix_name
                                ...
                                feature_reducer_hyperparameters_list <- p_classifier$feature_reducer_hyperparameters_list
                           ...
                           ## WARNING:  DO **NOT** ALTER THE ORDER OF THESE LIST ELEMENTS:
                            list(performance_results=data.frame(c(classifier_ret_list,
                                                 list(classifier_name=classifier_function_name,
                                                      ...
                                                      classifier_set_name=p_classifier$classifier_set_name,
...
g_run_mult_classifs_mult_hyperparams_cv_2 <- function(p_classifier_list,
                                                      p_classifier_functions_list,
                                                      p_classifier_feature_matrices_list,
                                                      p_case_label_vec,
                                                      p_num_cv_replications=1,
    ...
    work_list <- setNames(lapply(as.list(data.frame(t(data.frame(expand.grid(1:length(replications_folds_list_list),
                                                                             1:length(p_classifier_list)))))),
                                 setNames, c("replication_fold_id","classifier_id")), NULL)

    ml_global_results_list <- p_func_lapply(work_list,
                                            function(p_vec_inds_work) {
                                                replication_fold_id <- p_vec_inds_work["replication_fold_id"]
                                                classifier_id <- p_vec_inds_work["classifier_id"]
                                                replication_fold_list <- replications_folds_list_list[[replication_fold_id]]
                                                replication_id <- replication_fold_list$replication_id
                                                fold_id <- replication_fold_list$fold_id
                                                fold_assignments <- replications_fold_assignments_list[[replication_id]]
                                                inds_cases_test <- which(fold_id == fold_assignments)
...
g_make_classifier_function_xgboost <- function(p_nthread=1,
    ...
    function(p_classifier_feature_matrix,
        ...
        ## ==================== SAVE FOR CUSTOM OBJECTIVE FUNCTION DEVELOPMENT ==================
        ## if (is.null(p_case_group_ids)) {
        ## 
        ##     }
        ## } else {
        ##     custom_objective_for_training_grad <- function(preds, dtrain) {
        ##         labels <- getinfo(dtrain, "label")
        ##         my_loss_function_gradient_values <- preds - labels
        ##          ##  calculate your loss function gradient here
        ##         list(grad=my_loss_function_gradient_values, hess=rep(1, length(labels)))1
        ##     }
        ## }
        ## ==================== SAVE FOR CUSTOM OBJECTIVE FUNCTION DEVELOPMENT ==================
